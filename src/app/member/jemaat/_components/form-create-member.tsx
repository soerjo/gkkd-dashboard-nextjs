...
const FormSchema = z
    .object({
        full_name: z.string().min(1, { message: "required" }).max(110),
        name: z.string().min(1, { message: "required" }).max(110),
        email: z.string().min(1, { message: "required" }).max(110).email(),
        gender: z.string().min(1, { message: "required" }).max(110),
        place_birthday: z.string().min(1, { message: "required" }).max(110),
        date_birthday: z.date(),
        phone_number: z.string().regex(phoneRegex, 'Invalid Number!'),
        address: z.string().max(1100).optional(),
        father_name: z.string().max(110).optional(),
        mother_name: z.string().max(110).optional(),
        birth_order: z.number().min(0).optional(),
        total_brother_sister: z.number().min(0).optional(),
        marital_status: z.boolean().optional(),
        husband_wife_name: z.string().max(110).optional(),
