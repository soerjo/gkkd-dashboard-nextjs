...
const FormUpdateData = ({ id, onClose, data }: FormUpdateDataProps) => {
  ...
  const form = useForm<ICreateHospitalityData>({
...
export const UpdateFormDrawer = ({ id, onOpenChange, isOpen, data }: UpdateFormProps) => {
  return (
    <Drawer isOpen={isOpen} onOpenChange={onOpenChange} size="lg">
      <DrawerContent>
        {(onClose) => <FormUpdateData id={id} data={data} onClose={onClose} />}
      </DrawerContent>
    </Drawer>
  );
};
